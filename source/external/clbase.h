...
#define CHECK_CL(clStatus) if(clStatus != CL_SUCCESS) { std::cout << "File: " << __FILE__ << " " << "Line: " << __LINE__ << " " << "OpenCL error: " << openclGetErrorString(clStatus); }
...
inline std::vector<cl_context_properties> createContextProps(cl_platform_id selectedPlatformID)
{
...
public:
    ...
    void checkInteropInfo()
    {
        ...
        std::vector<cl_context_properties> clContextPropsInterop = createContextProps(platfromID);
...
public:
    ...
    void selectePlatformDevice_FirstCPU(cl_uint& selectedPlatformIndex, cl_uint& selectedDeviceIndex, cl_platform_id& selectedPlatformID, cl_device_id& selectedDeviceID)
    {
        for (cl_uint p = 0; p < numPlatforms; ++p)
        {
            for (cl_uint d = 0; d < platforms[p].numDevices; ++d)
            {
                if (platforms[p].devices[d].isCPU())
                {
                    selectedPlatformIndex = p;
                    selectedDeviceIndex = d;

                    selectedPlatformID = platfromIDs[p];
                    selectedDeviceID = platforms[p].devices[d].deviceID;
    ...
    void selectePlatformDevice_FirstGPU(cl_uint& selectedPlatformIndex, cl_uint& selectedDeviceIndex, cl_platform_id& selectedPlatformID, cl_device_id& selectedDeviceID)
    {
        for (cl_uint p = 0; p < numPlatforms; ++p)
        {
            for (cl_uint d = 0; d < platforms[p].numDevices; ++d)
            {
                if (platforms[p].devices[d].isGPU())
                {
                    selectedPlatformIndex = p;
                    selectedDeviceIndex = d;
